...
public class StaggeredGridView extends ExtendableListView {
	...
	private int mHeight[] = new int[200];
...
//			}
    ...
    private int getCachedChildHeight(int position) {
    	if (mHeight.length <= position) {
    		return 0;
    	}
    	return mHeight[position];
    }
    
    private void setCachedChildHeight(int position, int value) {
    	if (mHeight.length <= position) {
    		mHeight = Arrays.copyOf(mHeight, mHeight.length + 200);
    	}
    	mHeight[position] = value;
    }
    ...
    private void layoutGridChild(final View child, final int position,
		...
		int childH = getChildHeight(child);
		if (DBG) {
			if (childH != getCachedChildHeight(position) && getCachedChildHeight(position) != 0) {
				Log.e("top", "position " + position + "," + getCachedChildHeight(position) + "," + childH);
			} else {
				Log.v("top", "position " + position + "," + childH);
			}
		}
		setCachedChildHeight(position, childH);
		if (flowDown) {
			gridChildTop = mColumnBottoms[column]; // the next items top is the last items bottom
			gridChildBottom = gridChildTop + (childH + verticalMargins);
        }
        else {
			gridChildBottom = mColumnTops[column]; // the bottom of the next column up is our top
			gridChildTop = gridChildBottom - (childH + verticalMargins);
		}
	...
	private void updateColumnTopIfNeeded(int column, int childTop) {
		if (childTop < mColumnTops[column]) {
			...
			checkTop("updateColumnTopIfNeeded");
		}
	}
	...
	protected void offsetChildrenTopAndBottom(final int offset, final int column) {
		...
		checkTop("offsetChildrenTopAndBottom start");
        final int count = getChildCount();
        for (int i = 0; i < count; i++) {
            final View v = getChildAt(i);
            if (v != null &&
		...
		checkTop("offsetChildrenTopAndBottom");
	}
	...
	private void offsetAllColumnsTopAndBottom(final int offset) {
		if (offset != 0) {
			checkTop("offsetAllColumnsTopAndBottom start");
			for (int i = 0; i < mColumnCount; i++) {
				offsetColumnTopAndBottom(offset, i);
			}
			checkTop("offsetAllColumnsTopAndBottom end");
		}
	}

	private void checkTop(String func) {
		int topPos[] = new int[mColumnCount];
		Arrays.fill(topPos, -1);
		int topSum[] = new int[mColumnCount];
		Arrays.fill(topSum, 0);
		...
		for (int i = 0; i < getChildCount(); ++i) {
			...
			if (childParams.viewType != ITEM_VIEW_TYPE_HEADER_OR_FOOTER && childParams instanceof GridLayoutParams) {
				...
				if (column >= 0 && column < mColumnCount) {
					if (topPos[column] == -1 || position < topPos[column]) {
						topPos[column] = position;
					}
				}
			}
		}
		for (int column = 0; column < mColumnCount; ++column) {
			for (int i = getHeaderViewsCount(); i < topPos[column]; ++i) {
				if (getPositionColumn(i) == column) {
					int childTopMargin = getChildTopMargin(i);
					int childBottomMargin = getChildBottomMargin();
					int verticalMargins = childTopMargin + childBottomMargin + getCachedChildHeight(i);
					topSum[column] += verticalMargins;
				}
			}
		}
		
		for (int i = 1; i < mColumnCount; ++i) {
			int value = mColumnTops[0] - topSum[0] + topSum[i];
			if (value != mColumnTops[i]) {
				if (DBG) {
					Log.e("top", String.format("%s diff %d,%d, modify", func, mColumnTops[0] - topSum[0], mColumnTops[i] - topSum[i]));
				}
				mColumnTops[i] = value;
			}
		}
	...
	private void offsetColumnTopAndBottom(final int offset, final int column) {
		if (offset != 0) {
			...
			// checkTop("offsetColumnTopAndBottom");
		}
	}
		...
		for (int i = 0; i < getChildCount(); i++) {
			...
			if (child != null) {
                ...
                if (childParams.viewType != ITEM_VIEW_TYPE_HEADER_OR_FOOTER &&
					...
					int t = childTop - getChildTopMargin(position);
					if (t < mColumnTops[column]) {
						mColumnTops[column] = t;
					}
					final int childBottom = child.getBottom();
					int b = childBottom + getChildBottomMargin();
					if (b > mColumnBottoms[column]) {
						mColumnBottoms[column] = b;
					}
                }
                else {
					...
					checkTop("onChildrenDetached2");
				}
			}
		}
		checkTop("onChildrenDetached");
	}
	...
	public void onRestoreInstanceState(Parcelable state) {
		...
		checkTop("onRestoreInstanceState");
		mColumnBottoms = new int[mColumnCount];
